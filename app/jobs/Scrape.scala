...
object Scrape {
  ...
  def scrapeApi = {
    try {
      ...
      case e: Exception => Logger.info("Failed in scraping Car2Go API, will try again next time.", e)
    }
  }
  ...
  def pushToS3 = {
    try {
      ...
      case e: Exception => Logger.info("Failed in uploading to S3, will try again with next run.", e)
    }
  }
